...
public class emailsystem {
	...
	// public static String directory = "/Users/xiaosizhe/Desktop/demo/"; //
	// file directory
	...
	public static void readfile(String fileName) throws Exception {
		...
		// String filelocation ="/Users/xiaosizhe/Desktop/demo/"; //file
		// location
		File file2 = new File(directory + new SimpleDateFormat("yyyy-MM-dd").format(new Date()) + "_report.txt");
		BufferedWriter bw = new BufferedWriter(new FileWriter(file2));
		// String[] to = {
		// "database@gigasciencejournal.com","scott@gigasciencejournal.com","laurie@gigasciencejournal.com","chrisa@gigasciencejournal.com","maryann@gigasciencejournal.com"
		...
		for (Row row : sheet) {
			...
			// select rows
			if (row.getRowNum() > 104) // set to add more rows here is 100
										// default is 104
			{
				break;
			}
			...
			if (row.getRowNum() > 4) {
				...
				for (Cell cell : row) {
					...
					if (columnIndex == 16) {
						...
						// System.out.println ("Is shows Status change date as
						// show in Excel file: " + cell.getDateCellValue());
						if (record_date.after(current_date)) {
							System.out.println(
									"the row number for the date editorial status date : " + (row.getRowNum() + 1));
						}
					}
...

				{
					...
					// Get Data For Review=AssigningFTPbox Get DOI Ready for
					// GigaDB=Submitted ignore at this stage
					// db.comparestatus();
					bw.write(
							"---------------------------------------------update exist dataset begin ------------------------\n");
					...
					if (status_flag != null) {
						...
						// if(status_flag.equals("Request Data For Review") &&
						// !current_status.equals("Submitted") &&
						// !current_status.equals("AssigningFTPbox"))
						if (status_flag.equals("Request Data For Review") && rank_map.get(current_status.trim()) < 3) {
							...
							db.add_curation_log(return_dataset_id, log_date,
									"Status move from ImportFromEM upto AssigningFTPbox", "");
							db.update_dataset_status(return_dataset_id, "AssigningFTPbox");
							String print_message1 = "curation_log|" + return_dataset_id + "|" + log_date + "|" + "EM"
									+ "|" + "Status change" + "|" + "Status move from ImportFromEM upto AssigningFTPbox"
									+ "\n";
							bw.write(print_message1);
							String website_link = "http://gigadb-staging-jesse.gigatools.net/adminDataset/update/id/"
									+ return_dataset_id;
							String msgBody = "The manucript ID: " + manuscript_id
									+ " status changed to AssigningFTPbox, Pleasae check the dataset page "
									+ website_link;
							sendFromGMail(subject, msgBody);
						}
						// if(status_flag.equals("Get DOI Ready for GigaDB") &&
						// !current_status.equals("Submitted"))
						if (status_flag.equals("Get DOI Ready for GigaDB") && rank_map.get(current_status.trim()) < 7) {
							db.add_curation_log(return_dataset_id, log_date,
									"Status move from DataAvailableForReview upto Submitted", "");
							...
							String website_link = "http://gigadb-staging-jesse.gigatools.net/adminDataset/update/id/"
									+ return_dataset_id;
							String msgBody = "The manucript ID: " + manuscript_id
									+ " status changed to Submitted, Pleasae check the dataset page " + website_link;
							sendFromGMail(subject, msgBody);
				...
				String log_location = directory + new SimpleDateFormat("yyyy-MM-dd").format(new Date()) + "_"
						+ manuscript_id + "_report.txt";
				...
				if (tmp != 0) {
					...
					String print_ms_user = "gigadb_user|" + submitter_id + "|" + submitter_email + "|"
				...
				int image_id = db.add_image2("no_image.png", manuscript_id,
						"http://gigadb.org/images/data/cropped/no_image.png", "CC0", "GigaDB", "GigaDB");
				bw.write("image|" + "no_image.png" + "|" + manuscript_id + "|"
						+ "http://gigadb.org/images/data/cropped/no_image.png" + "|" + "CC0" + "|" + "GigaDB" + "|"
						+ "GigaDB" + "\n");
				...
				if (status_flag != null && status_flag.equals("Request Data For Review")) {
					db.add_dataset(submitter_id, manuscript_id, image_id, identifier, title, description, 1024,
							ftp_site, "AssigningFTPbox", 3, editor);
				} else {
					db.add_dataset(submitter_id, manuscript_id, image_id, identifier, title, description, 1024,
							ftp_site, "ImportFromEM", 3, editor);
				}
				insert_list.add(manuscript_id);
				bw.write("dataset|" + submitter_id + "|" + manuscript_id + "|" + image_id + "|" + identifier + "|"
						+ title + "|" + description + "|" + ftp_site + "|" + "Incomplete" + "\n");
				...
				for (int i = 0; i < aa.length; i++) {
					...
					String print_ms_author = "auther|" + author_id + "|" + surname + "|" + middle_name + "|"
							+ first_name + "\n";
				...
				if (funding_list != null && funding_list != " " && funding_list != "") {
					...
					for (int i = 0; i < funding.length; i++) {
						...
						if (temp != 0) {
							String print_ms_funder = "funding body already exists, id:" + temp + " name: "
									+ funding[i].trim() + "\n";
						...
						String print_ms_dataset_funder = "dataset_funder|" + funding[i].trim().replace("''", "'") + "|"
								+ grand[i].trim() + "|" + award[i].trim() + "\n";
				...
				String print_message = "curation_log|" + dataset_id + "|" + log_date + "|" + "EM" + "|"
						+ "Status updated to ImportFromEM" + "|" + "" + "\n";
				bw.write(print_message);
				bw1.write(print_message);
				if (curation_log != "" && curation_log != null) {

					db.add_curation_log(dataset_id, log_date, "notes from Eds", curation_log);
					String print_message1 = "curation_log|" + dataset_id + "|" + log_date + "|" + "EM" + "|"
							+ "notes from Eds" + "|" + curation_log + "\n";
					bw.write(print_message1);
					bw1.write(print_message1);
				}

				db.add_curation_log(dataset_id, log_date, "log file location", log_location);
				String print_message2 = "curation_log|" + dataset_id + "|" + log_date + "|" + "EM" + "|"
				...
				if (status_flag != null && status_flag.equals("Request Data For Review")) {
					...
					print_message = "curation_log|" + dataset_id + "|" + log_date + "|" + "EM" + "|"
							+ "Status move from ImportFromEM upto AssigningFTPbox" + "|" + "" + "\n";
					bw.write(print_message);
					bw1.write(print_message);
					String website_link = Setting.link + "adminDataset/update/id/" + dataset_id;
					String msgBody = "The manucript ID: " + manuscript_id
							+ " status changed to AssigningFTPbox, Pleasae check the dataset page " + website_link;
					sendFromGMail(subject, msgBody);
				}
	...
	public static void sendFromGMail(String subject, String body) {
		...
		try {
			...
			message.setRecipients(Message.RecipientType.TO, InternetAddress.parse(Setting.mailto));
			...
			if (pmm.getFrom().equals("GigaScience Editorial Office<em@editorialmanager.com>")
				...
				if (file_date.after(current_date)) {
					...
					for (int j = 0; j < multipart.getCount(); j++) {
						...
						File f = new File(directory + sdf.format(new Date()) + bodyPart.getFileName()); 																																																																											
